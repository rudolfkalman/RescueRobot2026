...
namespace ui_square_diff_drive
{
  ...
  SquareDiffDriveWidget::SquareDiffDriveWidget(QWidget * parent)
: rviz_common::Panel(parent)
{
  // UIの初期化
  ui.setupUi(this);
  // ボタンがクリックされたときの処理を設定
  //connect(ui.swerve_steer_push_button, &QPushButton::clicked, this, &SquareDiffDriveWidget::onPushButtonClicked);
}

void SquareDiffDriveWidget::onInitialize()
{
  ...
  qt_node_pub_handler_.initializePublisher("square_diff_drive_topic");
  ...
  timer_.start();
  connect(&timer_, &QTimer::timeout, this, &SquareDiffDriveWidget::onTimer);
}

void SquareDiffDriveWidget::onPushButtonClicked()
{
...
void SquareDiffDriveWidget::onTimer()
{
  // サブスクライバを使ってメッセージを受信
...
void SquareDiffDriveWidget::paintEvent(QPaintEvent * event)
{
  Q_UNUSED(event);
  ...
  painter.drawText(rect(), Qt::AlignTop | Qt::AlignHCenter, "Square Diff Drive");
...
PLUGINLIB_EXPORT_CLASS(ui_square_diff_drive::SquareDiffDriveWidget, rviz_common::Panel)
